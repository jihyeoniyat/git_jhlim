...
def main(
    ...
    ## 2. 일 발전량 조회 GET
    data = get_data(session, aliases, fromDate, toDate)
    ...
    file_name = f"kpx_history_{aliases}_{fromDate}_{toDate}.csv"
...
def get_data(session, aliases,fromDate, toDate):
    """
    - 3일 발전량 조회 데이터 호출
    """
...
def make_dataframe(data):
    ...
    list_tmp_df = []
    for n in range(13):
        location_name = data[n]["alias"]
        datetime = data[n]["dateTime"]
        datetime, _ = datetime.split('T')
        error_rate = data[n]["errorRate"]

        arr = []
        for temp in data[n]["data"]:
...
def load_csv(azure_key, df, aliases, fromDate, toDate):
