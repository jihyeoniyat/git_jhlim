...
#   1) Data Load                                                                                                           
#   2) Process : 파생변수 생성                    
#   3) Inference : 예측 모델 적용 및 결과 생성
#   --- 15 minutes later ---                                           
#   4) Comparison : 15분 후 실제 사용된 스팀량, 예측값
#   5) Storage                                    
#===============================================================================#
...
def compare_real_pred (raw, prediction) :

    # (1) 15분 후 실제 사용된 스팀량
    real = pd.DataFrame(columns= ['DateTime', 'Y_real'])
    real['DateTime'] = raw['DateTime']
    real['Y_real'] = raw['STEAM']

    # (2) 스토리지로 보낼 테이블
    result = pd.concat([real, prediction], axis=0, ignore_index=True) 
...
processed = processing_raw(raw)
prediction = pred_steamusage(raw, processed)
compare_real_pred(raw, prediction)
storage(prediction)
