import sys
import pandas as pd
import numpy as np
import sqlalchemy
import pymysql
import pymssql
from datetime import datetime, timedelta
import time

# ======================================================================================================================
# ======================================================================================================================
## Maria DB에서 데이터 Read

def select_maria_result(maria_engine, maria_tname, check_dt):

    maria_conn = maria_engine.connect() 

    query = """SELECT * FROM """ + maria_tname + """ WHERE dd = '""" + check_dt + """';"""

    result = pd.read_sql_query(query, maria_conn)

    return result

# ======================================================================================================================
# ======================================================================================================================
## DEFT_TYP_FF 컬럼 생성 & FF 오류메시지 별도 분리
## 2023-02-13 추가 (최종)
def FF_message_column(result1):

    is_FF = result1[result1['RST'] == 'FF']
    FF = is_FF[is_FF.columns[is_FF.columns.isin(['DEFT_TYP'])]]
    FF.rename(columns={'DEFT_TYP':'DEFT_TYP_FF'}, inplace=True)

    is_NG = result1[result1['RST'] == 'NG']
    NG = is_NG[is_NG.columns[is_NG.columns.isin(['DEFT_TYP'])]]

    merge_ff = pd.merge(result1, FF, how='left',left_index=True, right_index=True)
    merge_ff_ng = pd.merge(merge_ff, NG, how='left',left_index=True, right_index=True)
    merge_ff_ng = merge_ff_ng.drop(['DEFT_TYP_x'], axis = 1)
    merge_ff_ng.rename(columns={'DEFT_TYP_y':'DEFT_TYP'}, inplace=True)

    return merge_ff_ng

# ======================================================================================================================
# ======================================================================================================================
def set_line(line, result2):

    ## MS SQL 테이블에 들어갈 라인 정보
    ## 추가 라인 적용 시 추가 필요
    ## #2라인 2개 추가
    if line == 'MC22b_2LINE' :
        result2['LINE'] = 'MC22b_2'
    elif line == 'MC22b_3LINE' :
        result2['LINE'] = 'MC22b_3'
    elif line == 'MC22b_4LINE' :
        result2['LINE'] = 'MC22b_4'
    elif line == 'MR_1LINE' :
        result2['LINE'] = 'MR_1'
   
    # 하반기 라인 4개 추가    
    elif line == 'MC40a_1LINE' :
        result2['LINE'] = 'MC40a_1'
    elif line == 'MC40a_2LINE' :
        result2['LINE'] = 'MC40a_2'
    elif line == 'MC65a_1LINE' :
        result2['LINE'] = 'MC65a_1'
    elif line == 'MC100a_1LINE' :
        result2['LINE'] = 'MC100a_1'
 

    ## MS SQL 테이블의 필드 순서와 일치시켜주는 작업
    #result2 = result2.loc[:, ['LINE', 'DD', 'HHMISS', 'SRAL_ID', 'AMP', 'VOLT1', 'VOLT2', 'HZ1', 'HZ2', 'TYP', 'BRAND', 'BRAND_CAT', 'SIDE', 'RST', 'DEFT_TYP', 'DEFT_TYP_FF', 'X1CODI', 'Y1CODI', 'X2CODI', 'Y2CODI']]
    
    if line == 'MR_1LINE':
        result2 = result2.loc[:, ['LINE', 'DD', 'HHMISS', 'SRAL_ID', 'AMP', 'VOLT1', 'VOLT2', 'HZ1', 'HZ2', 'TYP', 'MR', 'BRAND','BRAND_CAT', 'SIDE', 'RST', 'DEFT_TYP', 'DEFT_TYP_FF', 'X1CODI', 'Y1CODI', 'X2CODI', 'Y2CODI']]

    elif (line == 'MC40a_1LINE' or line =='MC40a_2LINE') :
        result2 = result2.loc[:, ['LINE', 'DD', 'HHMISS', 'SRAL_ID', 'AMP', 'VOLT1', 'VOLT2', 'HZ1', 'HZ2', 'TYP', 'UA_1', 'BRAND','BRAND_CAT', 'SIDE', 'RST', 'DEFT_TYP','DEFT_TYP_FF', 'X1CODI', 'Y1CODI', 'X2CODI', 'Y2CODI']]

    elif (line == 'MC65a_1LINE' or line =='MC100a_1LINE') :
        result2 = result2.loc[:, ['LINE', 'DD', 'HHMISS', 'SRAL_ID', 'AMP', 'VOLT1', 'VOLT2', 'HZ1', 'HZ2', 'TYP', 'UA_1', 'TERMINAL', 'BRAND','BRAND_CAT', 'SIDE', 'RST', 'DEFT_TYP', 'DEFT_TYP_FF', 'X1CODI', 'Y1CODI', 'X2CODI', 'Y2CODI']]

    else:
        result2 = result2.loc[:, ['LINE', 'DD', 'HHMISS', 'SRAL_ID', 'AMP', 'VOLT1', 'VOLT2', 'HZ1', 'HZ2', 'TYP', 'BRAND', 'BRAND_CAT', 'SIDE', 'RST', 'DEFT_TYP', 'DEFT_TYP_FF', 'X1CODI', 'Y1CODI', 'X2CODI', 'Y2CODI']]


    return result2


# ======================================================================================================================
# ======================================================================================================================
## Vision Maria DB 접속 정보 (From)
maria_host = '165.244.138.138'
maria_user = 'lseadmin'
maria_password = 'Tbvjaosvkdnj00'
maria_db = 'mc'

maria_engine = sqlalchemy.create_engine('mysql+pymysql://' + maria_user + ':' + maria_password + '@' + maria_host
                                       + ':3306/' + maria_db + '?charset=utf8')

## Vision MSSQL DB 접속 정보 (To)
az_host = '20.194.52.255'
az_user = 'lseadmin'
az_password = 'Tbvjaosvkdnj00'
az_db = 'mc'

azure_engine = sqlalchemy.create_engine('mssql+pymssql://' + az_user + ':' + az_password + '@' + az_host
                                        + ':21433/' + az_db + '?charset=utf8')

## Line 리스트명
## 추가 라인이 적용될 경우 하기 리스트에 라인 명을 추가해줘야함
## 라인명은 반드시 각 라인 별 생성되는 테이블에 포함된 라인명과 일치해야함
## #2라인 2개 추가

#line_list = ['MC100a_1LINE']

line_list = ['MC22b_2LINE', 'MC22b_3LINE', 'MC22b_4LINE', 'MR_1LINE', 'MC40a_1LINE', 'MC40a_2LINE','MC65a_1LINE','MC100a_1LINE']

# D-1 날짜
## D-1 데이터를 배치로 가져오기 때문에 vat1 는 일반적으로 1
## 변수에 따라 D-n 의 데이터를 가져올 수 있음
var1 = sys.argv[1]
day_var = float(var1)
yesterday = datetime.now() - timedelta(days=day_var)
check_dt = yesterday.strftime('%Y%m%d')
check_mon = check_dt[:-2]

# ======================================================================================================================
# ======================================================================================================================
## 각 라인 별로 수행
for line in line_list:

    maria_tname = maria_db + '.ZMRA_SRFC_INSP_L_' + line + '_' + check_mon
    azure_tname1 = 'ZMRA_SRFC_INSP_L_' + line + '_' + check_mon
    azure_tname2 = 'ZMRA_SRFC_INSP_L'
    
    try:
        result1 = select_maria_result(maria_engine, maria_tname, check_dt)

    except:
        continue

    if not result1.empty:
        ## MS SQL 테이블의 데이터 타입과 일치시켜주는 작업
        result1['DD'] = result1['DD'].map(lambda x: pd.to_datetime(x, format='%Y-%m-%d'))
        result1['HHMISS'] = result1['HHMISS'].map(lambda x: ':'.join([x[0:2], x[2:4], x[4:]]))

        ## 전처리
        result_ff = FF_message_column(result1)

        ## 통합테이블 넣을 데이터 copy

        result2 = result_ff.copy()

        if line == 'MR_1LINE':
            result2 = set_line(line, result2)

        elif (line == 'MC40a_1LINE' or line =='MC40a_2LINE') :
            result2 = set_line(line, result2)
        
        elif (line == 'MC65a_1LINE' or line =='MC100a_1LINE') :
            result2 = set_line(line, result2)
        
        else:
            result2['MR'] = None
            result2 = set_line(line, result2)

        azure_conn = azure_engine.connect()


        try:
            ## 월이 변경되어 신규 테이블 생성이 필요한 경우(라인/월별로 테이블이 존재하기 때문에)
            if not azure_engine.dialect.has_table(azure_engine, azure_tname1):
                if line == 'MR_1LINE':
                    create_query = """CREATE TABLE [dbo].[""" + azure_tname1 + """](
                                           [ID] [int] IDENTITY(1,1) PRIMARY KEY NOT NULL,
                                           [DD] [date] NOT NULL,
                                           [HHMISS] [time](0) NOT NULL,
                                           [SRAL_ID] [varchar](50) NULL,
                                           [AMP] [varchar](3) NULL,
                                           [VOLT1] [varchar](3) NULL,
                                           [VOLT2] [varchar](3) NULL,
                                           [HZ1] [varchar](2) NULL,
                                           [HZ2] [varchar](2) NULL,
                                           [TYP] [varchar](8) NULL,
                                           [MR] [varchar](4) NULL,
                                           [BRAND] [varchar](8) NULL,
                                           [BRAND_CAT] [varchar](8) NULL,
                                           [SIDE] [varchar](16) NULL,
                                           [RST] [varchar](2) NULL,
                                           [DEFT_TYP] [varchar](255) NULL,
                                           [DEFT_TYP_FF] [varchar](255) NULL,
                                           [X1CODI] [int] NULL,
                                           [Y1CODI] [int] NULL,
                                           [X2CODI] [int] NULL,
                                           [Y2CODI] [int] NULL);"""

                elif (line == 'MC40a_1LINE' or line =='MC40a_2LINE') :
                    create_query = """CREATE TABLE [dbo].[""" + azure_tname1 + """](
                                           [ID] [int] IDENTITY(1,1) PRIMARY KEY NOT NULL,
                                           [DD] [date] NOT NULL,
                                           [HHMISS] [time](0) NOT NULL,
                                           [SRAL_ID] [varchar](50) NULL,
                                           [AMP] [varchar](3) NULL,
                                           [VOLT1] [varchar](3) NULL,
                                           [VOLT2] [varchar](3) NULL,
                                           [HZ1] [varchar](2) NULL,
                                           [HZ2] [varchar](2) NULL,
                                           [TYP] [varchar](8) NULL,
                                           [UA_1] [varchar](1) NULL,
                                           [BRAND] [varchar](8) NULL,
                                           [BRAND_CAT] [varchar](8) NULL,
                                           [SIDE] [varchar](16) NULL,
                                           [RST] [varchar](2) NULL,
                                           [DEFT_TYP] [varchar](255) NULL,
                                           [DEFT_TYP_FF] [varchar](255) NULL,
                                           [X1CODI] [int] NULL,
                                           [Y1CODI] [int] NULL,
                                           [X2CODI] [int] NULL,
                                           [Y2CODI] [int] NULL);"""

                elif (line == 'MC65a_1LINE' or line =='MC100a_1LINE') :
                    create_query = """CREATE TABLE [dbo].[""" + azure_tname1 + """](
                                           [ID] [int] IDENTITY(1,1) PRIMARY KEY NOT NULL,
                                           [DD] [date] NOT NULL,
                                           [HHMISS] [time](0) NOT NULL,
                                           [SRAL_ID] [varchar](50) NULL,
                                           [AMP] [varchar](3) NULL,
                                           [VOLT1] [varchar](3) NULL,
                                           [VOLT2] [varchar](3) NULL,
                                           [HZ1] [varchar](2) NULL,
                                           [HZ2] [varchar](2) NULL,
                                           [TYP] [varchar](8) NULL,
                                           [UA_1] [varchar](3) NULL,
                                           [TERMINAL] [varchar](8) NULL,
                                           [BRAND] [varchar](8) NULL,
                                           [BRAND_CAT] [varchar](8) NULL,
                                           [SIDE] [varchar](16) NULL,
                                           [RST] [varchar](2) NULL,
                                           [DEFT_TYP] [varchar](255) NULL,
                                           [DEFT_TYP_FF] [varchar](255) NULL,
                                           [X1CODI] [int] NULL,
                                           [Y1CODI] [int] NULL,
                                           [X2CODI] [int] NULL,
                                           [Y2CODI] [int] NULL);"""                                   
                else:
                    create_query = """CREATE TABLE [dbo].[""" + azure_tname1 + """](
                                           [ID] [int] IDENTITY(1,1) PRIMARY KEY NOT NULL,
                                           [DD] [date] NOT NULL,
                                           [HHMISS] [time](0) NOT NULL,
                                           [SRAL_ID] [varchar](50) NULL,
                                           [AMP] [varchar](3) NULL,
                                           [VOLT1] [varchar](3) NULL,
                                           [VOLT2] [varchar](3) NULL,
                                           [HZ1] [varchar](2) NULL,
                                           [HZ2] [varchar](2) NULL,
                                           [TYP] [varchar](8) NULL,
                                           [BRAND] [varchar](8) NULL,
                                           [BRAND_CAT] [varchar](8) NULL,
                                           [SIDE] [varchar](16) NULL,
                                           [RST] [varchar](2) NULL,
                                           [DEFT_TYP] [varchar](255) NULL,
                                           [DEFT_TYP_FF] [varchar](255) NULL,
                                           [X1CODI] [int] NULL,
                                           [Y1CODI] [int] NULL,
                                           [X2CODI] [int] NULL,
                                           [Y2CODI] [int] NULL);"""

                azure_conn.execute(create_query)

            ## 라인/월 별 테이블
            result_ff.to_sql(con=azure_engine, name=azure_tname1, index=False, if_exists='append')

            ## 통합 테이블
            try:
                result2.to_sql(con=azure_engine, name=azure_tname2, index=False, if_exists='append')
            except Exception as e :
                print(e)
        except Exception as e :
            print(e)
            continue

        else:
            print(line + ' ' + check_dt + ' Insert Complete')
