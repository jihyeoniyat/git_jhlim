...
# ======================================================================================================================
# ======================================================================================================================
def bottom_copy(ng_path, rst_save_path):
    bottom_path = ng_path + 'BOTTOM'
    b_path_list = []

    for root, dirs, files in os.walk(bottom_path):		
        if files:
            condition = check_dt + '*.*.*.png'
            condition_path = os.path.join(root, condition)
            b_files = glob.glob(condition_path)
            for file in b_files:
                b_path_list.append(file.replace("\\", "/"))
    
    for b_path in b_path_list:
        split_name = b_path.split('/')[-1].split('(')[0]
        rst_name = rst_save_path + '/BOTTOM/' + split_name + '.BOTTOM.RAIL_SPRING.0_0_0_0.jpg'
        im = Image.open(b_path).convert('RGB')
        os.makedirs(f'{rst_save_path}/BOTTOM', exist_ok=True)
        im.save(rst_name, 'jpeg')
...
def img_bounding(df, rst_save_path):
    ...
    for one_png_path, one_png_name, one_xml_path, one_xml_name in zip(png_path, png_name, xml_path, xml_name):
        ...
        tree = ET.parse(content_xml) # bohyun
        note = tree.getroot()
...
for line in line_list:
    ...
    bottom_copy(ng_path, rst_save_path)
